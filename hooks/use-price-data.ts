...
export function usePriceData(selectedSymbol: string) {
    ...
    const fetchTickerData = async () => {
      try {
        ...
        if (data) {
          setPriceData((prev) => ({
            ...prev,
            priceChange: data.priceChange ? Number.parseFloat(data.priceChange) : prev.priceChange,
            priceChangePercent: data.priceChangePercent
              ? Number.parseFloat(data.priceChangePercent)
              : prev.priceChangePercent,
            highPrice: data.highPrice ? formatPrice(data.highPrice) : prev.highPrice,
                  ...
                  data.c && prev.currentPrice
                    ? Number(data.c) > Number(prev.currentPrice)
                      ...
                      : Number(data.c) < Number(prev.currentPrice)
                        ...
                        : prev.priceDirection
                    : prev.priceDirection,
                priceChange: data.p ? Number.parseFloat(data.p) : prev.priceChange,
                priceChangePercent: data.P ? Number.parseFloat(data.P) : prev.priceChangePercent,
  ...
  useEffect(() => {
    ...
    const fetchOpenInterest = async () => {
      try {
        const response = await fetch(`https://fapi.binance.com/fapi/v1/openInterest?symbol=${selectedSymbol}`)
        const data = await response.json()
  ...
  useEffect(() => {
    ...
    const fetchFundingData = async () => {
      try {
        const response = await fetch(`https://fapi.binance.com/fapi/v1/premiumIndex?symbol=${selectedSymbol}`)
        const data = await response.json()

        if (!isActive) return

        if (data) {
          const rate = data.lastFundingRate ? Number(data.lastFundingRate) * 100 : 0
    ...
    const connectFundingWs = () => {
      ...
      try {
        const ws = new WebSocket(`wss://fstream.binance.com/ws/${selectedSymbol.toLowerCase()}@markPrice`)
