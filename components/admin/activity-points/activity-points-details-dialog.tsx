...
export function ActivityPointsDetailsDialog({
  ...
  const getContentDescription = (activity: ActivityPoint) => {
    ...
    switch (activity.transaction_type) {
      ...
      case "daily_login":
        return `Received daily login bonus${metadata.streak ? ` (Day ${metadata.streak})` : ""}`;
      default:
  ...
  const getRelatedContent = () => {
    ...
    switch (activity.transaction_type) {
      ...
      case "post_share":
        return {
          title: metadata.post_title || "Unknown Post",
      ...
      case "comment_add":
        return {
          title: metadata.post_title || "Unknown Post",
          id: metadata.post_id || activity.reference_id,
          commentId: activity.reference_id,
          type: "comment",
        };
      case "welcome_bonus":
        return {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader className="sticky top-0 z-10 pb-4 border-b">
          ...
          <DialogDescription>
            Activity ID:{" "}
            <span className="font-mono">{activity.act_id || activity.id}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4 flex-1 overflow-y-auto">
            ...
            <div>
              ...
              <div className="bg-muted/50 p-4 rounded-md">
                ...
                <div className="flex justify-between py-2 border-b border-gray-200 dark:border-gray-700">
                  ...
                  <span className="text-sm">{relatedContent.title}</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-sm font-medium">Content ID</span>
                  <span className="text-sm font-mono">{relatedContent.id}</span>
                </div>
                {relatedContent.type === "comment" &&
                  relatedContent.commentId && (
                    <div className="flex justify-between py-2 border-t border-gray-200 dark:border-gray-700">
                      <span className="text-sm font-medium">Comment ID</span>
                      <span className="text-sm font-mono">
                        {relatedContent.commentId}
                      </span>
                    </div>
                  )}
                {relatedContent.type === "bonus" && relatedContent.streak && (
                  <div className="flex justify-between py-2 border-t border-gray-200 dark:border-gray-700">
                    <span className="text-sm font-medium">Login Streak</span>
                    <span className="text-sm">Day {relatedContent.streak}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div>
            ...
            <div className="bg-muted/50 p-4 rounded-md">
              ...
              <div className="flex justify-between py-2">
                <span className="text-sm font-medium">Reference Type</span>
                <span className="text-sm">{activity.reference_type}</span>
              </div>
            </div>
          </div>
        </div>
